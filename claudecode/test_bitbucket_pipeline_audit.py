#!/usr/bin/env python3
"""
Pytest tests for Bitbucket Pipeline audit script components.
"""
...
class TestImports:
    ...
    def test_main_module_import(self):
        ...
        assert hasattr(bitbucket_pipeline_audit, 'SimpleClaudeRunner')
        assert hasattr(bitbucket_pipeline_audit, 'main')

    def test_bitbucket_client_import(self):
        """Test that BitbucketClient can be imported."""
        from claudecode.bitbucket_client import BitbucketClient
        assert BitbucketClient is not None
...
class TestHardExclusionRules:
    ...
    def test_markdown_file_exclusion(self):
        ...
        md_findings = [
            ...
            {'file': 'CHANGELOG.MD', 'description': 'XSS vulnerability'},
            {'file': 'path/to/file.Md', 'description': 'Path traversal'},
        ]
    ...
    def test_non_markdown_files_not_excluded(self):
        ...
        non_md_findings = [
            ...
            {'file': 'file.mdx', 'description': 'Security issue'},
        ]
...
class TestPromptsModule:
    ...
    def test_get_security_audit_prompt(self):
        ...
        # Verify we don't reference GitHub
        assert 'GitHub PR' not in prompt
...
class TestDeploymentPRDetection:
    ...
    def test_deployment_pr_patterns(self):
        ...
        deployment_titles = [
            ...
            "deploy abcdef123456 to staging",
            "DEPLOY ABCDEF01 TO DEVELOPMENT",
        ]

        for title in deployment_titles:
            assert re.match(deployment_pattern, title, re.IGNORECASE), f"Failed to match: {title}"
    ...
    def test_non_deployment_pr_patterns(self):
        ...
        non_deployment_titles = [
            ...
            "Deploy abc to production",
            "Deploy 12345g to production",
        ]

        for title in non_deployment_titles:
            assert not re.match(deployment_pattern, title, re.IGNORECASE), f"Incorrectly matched: {title}"
